...
typedef struct {
  ...
  char *command;                 /*  stores the high-level abstract command, e.g. "USER" or "PASS" in FTP, "DESCRIBE" or "SETUP" in RTSP */
} region_t;
...
extern unsigned int* (*extract_response_codes)(unsigned char* buf, unsigned int buf_size, unsigned int num_to_read, unsigned int* state_count_ref);
