...
unsigned int* (*extract_state_traversal)(region_t* regions, u32 region_count, Agraph_t *ipsm, unsigned int state_machine_state_cnt, unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref);
...
//      q->regions[i].state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);
      ...
      q->regions[i].state_sequence = extract_state_traversal(q->regions, q->region_count, ipsm, state_ids_count, response_buf, response_buf_size, &state_count);
...
void update_fuzzs(region_t *regions, u32 region_count) {
...
//  unsigned int *state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);
  unsigned int *state_sequence = extract_state_traversal(regions, region_count, ipsm, state_ids_count, response_buf, response_buf_size, &state_count);
...
u32 update_scores_and_select_next_state(u8 mode) {
  ...
  for(i = 0; i < state_ids_count; i++) {
    ...
    if (k != kh_end(khms_states)) {
      ...
      switch(mode) {
        case FAVOR:
          state->score = state->seeds_count == 0 ? 1 : ceil(1000 * pow(2, -log10(log10(state->fuzzs + 1) * state->selected_times + 1)) * pow(2, log(state->paths_discovered + 1)));
          if (state->score > 12000) state->score = 12000; // cap the score
          break;
...
//            if (result->generating_state_id != target_state_id && !result->is_initial_seed && UR(100) < 75) continue;

            // HJ: no reason to favour the initial seeds that much. This hurts us since we have more states, and fewer seeds at each state
            if (result->generating_state_id != target_state_id && UR(100) < 75) continue;
            ...
            if (pending_favored) {
              ...
              break;
            } else if (!result->favored && queued_paths > 5) {
...
//  unsigned int *state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);
  ...
  unsigned int *state_sequence = extract_state_traversal(q->regions,q->region_count, ipsm, state_ids_count, response_buf, response_buf_size, &state_count);
  ...
  if (is_state_sequence_interesting(state_sequence, state_count)) {
    ...
    u8 *temp_str = state_sequence_to_string(state_sequence, state_count);
		u8 *fname = alloc_printf("%s/replayable-new-ipsm-paths/id:%s:%s",
				out_dir, temp_str, dry_run ? basename(q->fname) : "new");
		...
		if (state_count > 1) {
			...
			for (i = 1; i < state_count; i++) {
				...
				if (!from) {
					...
					agset(from, "label", q->regions[i-1].command && strlen(q->regions[i-1].command) > 0 ? q->regions[i-1].command : "N/A?" );


					//Insert this newly discovered state into the states hashtable
					state_info_t *newState_From = (state_info_t*) ck_alloc(
							sizeof(state_info_t));
					...
					state_ids = (u32*) ck_realloc(state_ids,
							(state_ids_count + 1) * sizeof(u32));
				...
				if (!to) {
					...
					if (dry_run)
						...
						agset(to, "color", "red");
					char* node_label = "";
					if (q->regions[i-1].command && strlen(q->regions[i-1].command) > 0) {
						node_label = calloc(strlen(q->regions[i-1].command)+1, sizeof(char));
						strcpy(node_label, q->regions[i-1].command);
					}


					agset(to, "label", agstrdup(ipsm, node_label));

//					OKF("create state %d with label = %s", curStateID, node_label);

					//Insert this newly discovered state into the states hashtable
					state_info_t *newState_To = (state_info_t*) ck_alloc(
							sizeof(state_info_t));
...
//			    edge = agedge(ipsm, from, to, "new_edge", TRUE);
					...
					edge_label[0] = '\0';
					for (int j = 0;
							i >= 1 && q->regions[i - 1].command
									&& q->regions[i - 1].command[j] && i < 14;
							j++) {
						edge_label[j] = tolower(q->regions[i - 1].command[j]);
						edge_label[j + 1] = NULL;
					}
					edge = agedge(ipsm, from, to, edge_label, TRUE);

					if (i >= 1 && q->regions[i - 1].command) {

						agset(edge, "label", agstrdup(ipsm, edge_label));
  ...
  for(i = 0; i < q->region_count; i++) {
    ...
    if (regional_state_count > 0) {
      ...
      if (k != kh_end(khms_states)) {
        ...
        newState->fuzzs = 1000; // arbitrarily give a higher default to prevent these states from getting too high an energy
        newState->score = 1;
...
EXP_ST void destroy_queue(void) {
  ...
  while (q) {
    ...
    for (i = 0; i < q->region_count; i++) {
      ...
      if (q->regions[i].command) ck_free(q->regions[i].command);
    }
...
static void read_state_machine_nodes(void) {
	u8* fn;
	fn = alloc_printf("%s/preprocessed_states/nodes", in_dir);
	struct stat sta;

	if (lstat(fn, &sta) || access(fn, R_OK))
	...
	while ((read = getline(&line, &linelen, fp)) != -1) {
		...
		OKF("About to read line of size: %d \n", read);

		curStateID = strtol(line, NULL, 10);
		sprintf(line, "%d", curStateID);

		Agnode_t *to;
		to = agnode(ipsm, strdup(line), TRUE);

		agset(to,"color","green"); // green to denote info from predefined state machine

		OKF("About to create node of %s \n", strdup(line));

		state_info_t *newState_To = (state_info_t *) ck_alloc (sizeof(state_info_t));
		newState_To->id = curStateID;
		newState_To->is_covered = 0;
		newState_To->paths = 0;
		newState_To->paths_discovered = 0;
		newState_To->selected_times = 0;
		newState_To->fuzzs = 0;
...
//		free(line);
	...
	fclose(fp);
	expand_was_fuzzed_map(1, 0); // one more for state 0 (reserved by AFLnet for the first node already)

	OKF("Finished reading states \n");
}

static void add_state_0(void) {
	unsigned int curStateID = 0;
	khint_t k;

	Agnode_t *to;
	to = agnode(ipsm, "0", TRUE);
...
static void read_state_machine_edges(void) {
	u8* fn;
	fn = alloc_printf("%s/preprocessed_states/edges", in_dir);
	struct stat sta;
	...
	while ((read = getline(&line, &linelen, fp)) != -1) {
		...
		OKF("Reading transition: %s\n", line);


		// HJ notes for self: be careful over here. The ordering of (node) --edge-> (node) is different from what I usually use.
		first_state = strsep(&line, " ");
		edge_label = strsep(&line, " ");
		second_state = strsep(&line, "\n");


		Agnode_t *from, *to;
		Agedge_t *edge;
		from = agnode(ipsm, first_state, FALSE);
		if (from == NULL)
			PFATAL("Unable to access first node:%s'", first_state);
		to = agnode(ipsm, second_state, FALSE);

		if (to == NULL)
			PFATAL("Unable to access second node:%s'", second_state);
	  ...
	  //Add an edge to the graph
		  ...
		  agset(edge, "color", "green"); // use green to denote transitions given by our predefined state machine

		  agset(edge, "label", edge_label);
		}

		OKF("Finished reading transition: %s %s %s \n", first_state, edge_label, second_state);
	...
	OKF("Finished reading transitions \n");
}

static void read_state_machine(void) {
	// update the following varialbes
	//		kl_messages;
	//		khs_ipsm_paths;
	//		khms_states;
	// 		state_ids
	...
	// other than adding seeds, we wanna set the states too, (in the state machine)


	// always init state 0


//	read_state_machine_nodes();
	add_state_0();
//	read_state_machine_edges();

	// HJ: enable corpus_read_or_sync
	...
	// assume that in the ./states/ directory,
	// that for each state in the state machine, there is a <response_code>_<i>_<j> file containing the seed.
	// each <response_code>_<i> identifies a state from DICE, and <j> identify one seed of the state.
	u8* fn;
	fn = alloc_printf("%s/preprocessed", in_dir);

	nl_cnt = scandir(fn	, &nl, NULL, alphasort);

	OKF("Going to read seeds associated with each state	 \n");
	for (i = 0; i < nl_cnt; i++) {

		u8* fn = alloc_printf("%s/%s/%s", in_dir, "preprocessed", nl[i]->d_name);
		...
		char* name = calloc(strlen(nl[i]->d_name)+1, sizeof(char));
		char* orig = name;
		    strcpy (name, nl[i]->d_name);


		char * state_id_from_name = strsep(&name, "__");

	      // update seeds of state
//    	OKF("Going to fetch seed for state %s from file: %s. ", state_id_from_name, nl[i]->d_name);


    	unsigned int state_id = strtol(state_id_from_name, NULL, 10);
...
//		  state = kh_val(khms_states, k);
//		  state->seeds = (void **) ck_realloc (state->seeds, (state->seeds_count + 1) * sizeof(void *));
//		  state->seeds[state->seeds_count] = (void *)queue_top;
//		  state->seeds_count++;
//
////		  was_fuzzed_map[0][queued_paths - 1] = 0; //Mark it as reachable but not fuzzed
//
//
//		} else {
//		  PFATAL("AFLNet - the filename of the state machine for initialization might be wrong. We should be able to get a state id from the filename.");
//		}
	...
	SAYF("\n" cLRD "[*] " cRST
			"Done reading state machines\n"
			);
	OKF(
			"Total states initialized: %d \n", total_states);

	struct queue_entry* current_seed = queue;
	...
	OKF("Total seeds from states initialized: %d \n", total_seeds);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  update_fuzzs(regions, region_count);
    ...
    switch (opt) {
      ...
      case 'P': /* protocol to be tested */
        ...
        if (!strcmp(optarg, "RTSP")) {
          ...
          extract_state_traversal = &extract_state_traversal_rtsp;
        } else if (!strcmp(optarg, "FTP")) {
          extract_requests = &extract_requests_ftp;
          extract_state_traversal = &extract_state_traversal_ftp;
...
//  return 0;
  	// just wanna test reading the stuff first
